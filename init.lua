local il_editor = editor.editor:new("editor:editor")
editor_insecure_lua = { editor = il_editor }
il_editor:register_button("Run", function(self, name, context)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	if formname == "editor:editor" then
		...
		il_editor:on_event(name, fields)
	elseif formname == "editor:editor_new" then
		local name = player:get_player_name()
		il_editor:on_new_dialog_event(name, fields)
	end
end)
...
minetest.register_on_joinplayer(function(player)
	...
	if file then
		...
		file:close()
		il_editor._context[name].filesystem:load(datapath .. "/" .. name .. ".lua")
	else
...
minetest.register_on_shutdown(function()
	for key, value in pairs(il_editor._context) do
